...
private:
	...
	float totalTime = 0;				// Total time for FPS

	// ImGui buffers for input
    char inputPathBuffer[64];
    char outputPathBuffer[64];
    char outputFpsBuffer[64];
    int pathDuration;

	// FPS recording data
    bool isRecording;
    std::string recordingFilePath;
    std::vector<float> recordingTime;
    std::vector<float> recordingFps;
    int recordingCheckpoints;
    int recordingTimeSinceLastCheckpoint;
    int recordingAccumulatedTime;
};
